...
AndroidExternalViewEmbedder::AndroidExternalViewEmbedder(
    ...
    : ExternalViewEmbedder(),
      ...
      task_runners_(task_runners), current_processing_view_id_(kInvalidFlutterViewId)
       {}
...
void AndroidExternalViewEmbedder::PrerollCompositeEmbeddedView(
  ...
  FML_DCHECK(current_processing_view_id_ != kInvalidFlutterViewId);
  FML_DCHECK(flutter_view_group_datas_.find(current_processing_view_id_) != flutter_view_group_datas_.end());
  ...
  flutter_view_group_datas_[current_processing_view_id_]->composition_order_.push_back(view_id);
...
void AndroidExternalViewEmbedder::SubmitFlutterView(
  ...
  FML_DCHECK(flutter_view_id != kInvalidFlutterViewId);
  FML_DCHECK(flutter_view_group_datas_.find(flutter_view_id) != flutter_view_group_datas_.end());
  ...
  auto &group_data = flutter_view_group_datas_[flutter_view_id];
  const std::vector<int64_t>& composition_order = group_data->composition_order_;
  const int64_t previous_frame_view_count = group_data->previous_frame_view_count_;

  std::unordered_map<int64_t, SkRect> view_rects;
  for (auto platform_id : composition_order) {
  ...
  auto should_submit_current_frame = previous_frame_view_count > 0;
  ...
  for (int64_t view_id : composition_order) {
    ...
    std::unique_ptr<SurfaceFrame> frame =
        CreateSurfaceIfNeeded(group_data->surface_pool_.get(),
                              context,                    //
                              view_id,                    //
...
AndroidExternalViewEmbedder::CreateSurfaceIfNeeded(SurfacePool *surface_pool,
                                                  GrDirectContext* context,
                                                   int64_t view_id,
  ...
  // auto &surface_pool = flutter_view_group_datas_[current_processing_view_id_]->surface_pool_;

  std::shared_ptr<OverlayLayer> layer = surface_pool->GetLayer(
...
bool AndroidExternalViewEmbedder::FrameHasPlatformLayers() {
    FML_DCHECK(current_processing_view_id_ != kInvalidFlutterViewId);
  FML_DCHECK(flutter_view_group_datas_.find(current_processing_view_id_) != flutter_view_group_datas_.end());
  return !flutter_view_group_datas_[current_processing_view_id_]->composition_order_.empty();
}
...
void AndroidExternalViewEmbedder::Reset() {
  ...
  auto &group_data = flutter_view_group_datas_[current_processing_view_id_];
  group_data->previous_frame_view_count_ = group_data->composition_order_.size();
  for (auto &view_id : group_data->composition_order_) {
    slices_.erase(view_id);
  }

  group_data->composition_order_.clear();
  // slices_.clear();
}
...
current_processing_view_id_ = flutter_view_id;
auto it = flutter_view_group_datas_.find(flutter_view_id);
if (flutter_view_group_datas_.find(flutter_view_id) == flutter_view_group_datas_.end()) {
  flutter_view_group_datas_.emplace(flutter_view_id, std::make_unique<GroupData>());
}
auto &group_data = flutter_view_group_datas_[flutter_view_id];
// auto surface_pool = std::make_unique<SurfacePool>(/*use_new_surface_methods=*/false);
group_data->surface_pool_ = std::make_unique<SurfacePool>(/*use_new_surface_methods=*/false);

  // The surface size changed. Therefore, destroy existing surfaces as
  // the existing surfaces in the pool can't be recycled.
  if (group_data->frame_size_ != frame_size) {
    DestroySurfaces(group_data->surface_pool_.get());
  }

  group_data->surface_pool_->SetFrameSize(frame_size);
  // surface_pools_.emplace(flutter_view_id, std::move(surface_pool));

  group_data->frame_size_ = frame_size;
  group_data->device_pixel_ratio_ = device_pixel_ratio;
}
...
// |ExternalViewEmbedder|
void AndroidExternalViewEmbedder::Teardown(int64_t flutter_view_id) {
    FML_DCHECK(flutter_view_id != kInvalidFlutterViewId);
  FML_DCHECK(flutter_view_group_datas_.find(flutter_view_id) != flutter_view_group_datas_.end());
  DestroySurfaces(flutter_view_group_datas_[flutter_view_id]->surface_pool_.get());
}

// |ExternalViewEmbedder|
void AndroidExternalViewEmbedder::DestroySurfaces(SurfacePool *surface_pool) {
  if (!surface_pool->HasLayers()) {
    return;
  }
  fml::AutoResetWaitableEvent latch;
  fml::TaskRunner::RunNowOrPostTask(task_runners_.GetPlatformTaskRunner(),
                                    [&]() {
                                      surface_pool->DestroyLayers(jni_facade_);
                                      latch.Signal();
                                    });
  latch.Wait();
}
