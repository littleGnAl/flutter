...
//  public:
  AndroidPlatformViewController::AndroidPlatformViewController(
      ...
      const TaskRunners& task_runners): ExternalViewEmbedder(),
  ...
  // |ExternalViewEmbedder|
  DlCanvas* AndroidPlatformViewController::CompositeEmbeddedView(int64_t flutter_view_id, int64_t view_id)  {
  ...
  void AndroidPlatformViewController::PrepareFlutterView(int64_t flutter_view_id,
    ...
    if (frame_size_ != frame_size) {
      DestroySurfaces(flutter_view_id);
    }
...
void AndroidExternalViewEmbedder::PrerollCompositeEmbeddedView(
  ...
  if (iter != platform_view_controllers_.end()) {
    ...
    platform_view_controller->PrerollCompositeEmbeddedView(flutter_view_id, view_id, std::move(params));
  }
}

// |ExternalViewEmbedder|
DlCanvas* AndroidExternalViewEmbedder::CompositeEmbeddedView(int64_t flutter_view_id, int64_t view_id) {
  // if (slices_.count(view_id) == 1) {
  //   return slices_.at(view_id)->canvas();
  // }
  ...
  if (iter != platform_view_controllers_.end()) {
    AndroidPlatformViewController *platform_view_controller = iter->second.get();
    platform_view_controller->CompositeEmbeddedView(flutter_view_id, view_id);
  }
...
PostPrerollResult AndroidExternalViewEmbedder::PostPrerollAction(
  ...
  if (iter != platform_view_controllers_.end()) {
    AndroidPlatformViewController *platform_view_controller = iter->second.get();
    return platform_view_controller->PostPrerollAction(flutter_view_id, raster_thread_merger);
  }
...
void AndroidExternalViewEmbedder::PrepareFlutterView(
  ...
  platform_view_controller->PrepareFlutterView(flutter_view_id, frame_size, device_pixel_ratio);
}
