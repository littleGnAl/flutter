...
class AndroidExternalViewEmbedder final : public ExternalViewEmbedder {
 public:
  ...
  // |ExternalViewEmbedder|
  void Teardown(int64_t flutter_view_id) override;
 ...
 private:
  ...
  const std::unordered_map<int64_t, std::unique_ptr<SurfacePool>> surface_pools_;
  ...
  int64_t current_processing_view_id_;

  std::unordered_map<int64_t, std::unique_ptr<GroupData>> flutter_view_group_datas_;
  ...
  void DestroySurfaces(SurfacePool *surface_pool);
  ...
  std::unique_ptr<SurfaceFrame> CreateSurfaceIfNeeded(SurfacePool *surface_pool,
                                                        GrDirectContext* context,
                                                      int64_t view_id,
