...
- (void)beginFrameWithSize:(flutter::DlISize)frameSize {
  ...
  // self.currentProcessingFlutterViewId = flutterViewId;
...
- (void)prerollCompositeEmbeddedView:(int64_t)viewId
  ...
  // self.platformViewIdToFlutterViewIdMapping.emplace(viewId, self.currentProcessingFlutterViewId);
...
//  self.hadPlatformViews = !self.compositionOrder.empty();
  self.flutterViewHadPlatformViews[flutterViewId] = !self.compositionOrder.empty();// !currentFrameCompositionOrder.empty();
