...
#include "flutter/shell/gpu/gpu_surface_multiple_surfaces.h"
...
GPUSurfaceMultipleSurfaces::GPUSurfaceMultipleSurfaces(
  ...
  FML_LOG(ERROR) << "GPUSurfaceMultipleSurfaces::GPUSurfaceMultipleSurfaces context" << (context == nullptr);

  FML_LOG(ERROR) << "GPUSurfaceMultipleSurfaces::GPUSurfaceMultipleSurfaces context->IsValid()" << (context && context->IsValid());
...
// |Surface|
GPUSurfaceMultipleSurfaces::~GPUSurfaceMultipleSurfaces() = default;

// |Surface|
bool GPUSurfaceMultipleSurfaces::IsValid() {
  return is_valid_;
}

// |Surface|
std::unique_ptr<SurfaceFrame> GPUSurfaceMultipleSurfaces::AcquireFrame(
    const SkISize& size) {
  if (!IsValid()) {
...
// |Surface|
SkMatrix GPUSurfaceMultipleSurfaces::GetRootTransformation() const {
...
// |Surface|
GrDirectContext* GPUSurfaceMultipleSurfaces::GetContext() {
  // Impeller != Skia.
  return nullptr;
}
...
GPUSurfaceMultipleSurfaces::MakeRenderContextCurrent() {
  return delegate_->GLContextMakeCurrent();
}

// |Surface|
bool GPUSurfaceMultipleSurfaces::ClearRenderContext() {
  return delegate_->GLContextClearCurrent();
}

bool GPUSurfaceMultipleSurfaces::AllowsDrawingWhenGpuDisabled() const {
  return delegate_->AllowsDrawingWhenGpuDisabled();
}

// |Surface|
bool GPUSurfaceMultipleSurfaces::EnableRasterCache() const {
  return false;
}

// |Surface|
std::shared_ptr<impeller::AiksContext> GPUSurfaceMultipleSurfaces::GetAiksContext()
    const {
