...
GPUSurfaceGLImpeller::GPUSurfaceGLImpeller(
  ...
  if (!aiks_context->IsValid()) {
    FML_LOG(ERROR) << "GPUSurfaceGLImpeller::GPUSurfaceGLImpeller aiks_context->IsValid()" << (aiks_context->IsValid());
    return;
  }
...
// ): GPUSurfaceGLImpeller(nullptr, context, render_to_surface),
//   get_gpu_surface_delegate_(get_gpu_surface_delegate) {
//   // get_gpu_surface_delegate_ = get_gpu_surface_delegate;
//  }
...
delegate =delegate_;
  ...
  // if (!delegate) {
  //   FML_LOG(ERROR) << "fail to get delegate view_id: " << view_id;
  //   return nullptr;
  // } else {
  //   FML_LOG(ERROR) << "got delegate view_id: " << view_id;
  // }
